...
import numpy as np  # Für Regressionsberechnung
import time
...
try:
    ...
    plt.scatter(timestamps, values, label="GSR Werte", color="blue", alpha=0.7)

    # Berechne die Regressionsgerade
    if len(timestamps) > 1:
        coefficients = np.polyfit(timestamps, values, 1)  # Lineare Regression
        poly_eq = np.poly1d(coefficients)
        regression_values = poly_eq(timestamps)

        # Zeichne die Regressionsgerade
        plt.plot(timestamps, regression_values, label="Regressionsgerade", color="red", linewidth=2)

    # Beschrifte den Plot
    plt.title("GSR Sensor Data mit Regressionsgerade")
    plt.xlabel("Time (s)")
    ...
    plt.savefig("gsr_plot_regression.png")
    print "Plot gespeichert als gsr_plot_regression.png"
